...
def calculate_feature_vector(post_dictionary):
    ...
    style_feature_vector = [total_words, average_sentence_length, I_count, exclamation_count, ellipsis_count, misspellings_count]
    feature_vector = style_feature_vector + normalized_word_frequency_vector 
    return (feature_vector, url, excerpt.encode("utf8"), title.encode("utf8"), domain)
...
if __name__ == "__main__":
    try:
        for post_dictionary in call_readability():
            calculate_feature_vector(post_dictionary)
    except Exception, e:
